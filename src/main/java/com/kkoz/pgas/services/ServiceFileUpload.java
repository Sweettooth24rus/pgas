...
import com.kkoz.pgas.config.FileStorageConfig;
import com.kkoz.pgas.controls.exceptions.CannotDeleteFileException;
import com.kkoz.pgas.controls.exceptions.CannotShowFileException;
import com.kkoz.pgas.controls.exceptions.FileNotExistsException;
...
import com.kkoz.pgas.entities.dictionaries.EntityWithFiles;
...
import com.kkoz.pgas.repositories.RepoFileUpload;
...
public class ServiceFileUpload {
    ...
    public String deleteFromApplication(DtoFileDelete dtoFileDelete) {
        return this.delete(dtoFileDelete.getEntityId(), dtoFileDelete.getUuid());
    }

    public void deleteIfUuidEquals(ApplicationFile elem, UUID uuid, Application entity) {
        if (elem.getFile().getUuid().equals(uuid)) {
            elem.setFile(null);
            //repo
        }
    }

    private String delete(Integer entityId, UUID uuid) {
        if (entityId != null) {
            if (repoFileUpload.findByUuid(uuid).isPresent()) {

                Application entity = repoApplication.getById(entityId);

                entity.getApplicationFileList().stream()
                        .forEach(elem -> deleteIfUuidEquals(elem, uuid, entity));

                /*if (entityName.equals(EntityWithFiles.RECIPE.getKey())) {
                    //repoRecipe.save((Application) entity);
                }*/
